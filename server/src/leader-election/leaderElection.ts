...
function failureDetector(leaderPort: number): Promise<boolean> {
    return new Promise((resolve) => {
        ...
        if(!wsURL){
            console.error(`Cant find wsURL from peerport mapping, PORT : ${leaderPort}`);
            return;
        }
...
function initiateElection(): void {
    ...
    PEER_PORTS.forEach((peerPort) => {
        if (peerPort > PORT) {
            ...
            if(!wsURL){
                console.error(`Cant find wsURL from peerport mapping, PORT : ${peerPort}`);
                return;
            }
            const ws = new WebSocket(wsURL);
...
export async function manageDatabaseCluster(){
    ...
    if(isLeader){
        ...
        if(dbPrimaryHealth != undefined)
            isPrimaryDBAlive = dbPrimaryHealth;
        if(dbReplicaHealth != undefined)
            isReplicaDBAlive = dbReplicaHealth;
